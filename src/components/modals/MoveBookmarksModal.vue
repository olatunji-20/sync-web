    ...
    >
      ...
      >
        ...
        <div class="flex items-start justify-between gap-4 px-6 pt-6 pb-4">
          <div>
            <h2 id="move-bookmarks-title" class="text-lg font-bold text-stone-900">Move bookmarks</h2>
        ...
        <!-- Collection list -->
        <div class="px-6 pb-2 max-h-64 overflow-y-auto">
          <button
            v-for="col in collections"
            :key="col.id"
            type="button"
            class="w-full flex items-center gap-4 p-3 rounded-xl border text-left transition-colors"
            :class="selectedId === col.id ? 'border-[#6B4CF5] bg-[#6B4CF5]/5' : 'border-stone-200 hover:bg-stone-50'"
            @click="selectedId = col.id"
          >
            <div
              class="w-12 h-12 rounded-lg shrink-0 flex items-center justify-center text-xl"
              :class="col.thumbnailClass"
            >
              {{ col.icon }}
            </div>
            <div class="min-w-0 flex-1">
              <p class="font-medium text-stone-900">{{ col.name }}</p>
        ...
        <div class="flex justify-end px-6 py-4 border-t border-stone-100">
          <button
            type="button"
            class="px-5 py-2.5 rounded-xl bg-[#6B4CF5] text-white font-medium hover:bg-[#6B4CF5]/90 transition-colors"
            @click="onMove"
          >
            Move
          </button>
