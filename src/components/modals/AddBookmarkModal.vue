<template>
  <Teleport to="body">
    <div v-show="visible" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/50"
      ...
      <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden" role="dialog" aria-modal="true"
        aria-labelledby="add-bookmark-title">
        <!-- Header -->
        <div class="text-center gap-4 p-6">
          <div>
            <div class="size-[40px] rounded-full mb-3 flex items-center justify-center mx-auto bg-stone-100">
            ...
            <h2 id="add-bookmark-title" class="text-lg font-semibold text-stone-900">Add bookmark manually</h2>
...
const stepLabel = computed(() =>
  isFetching.value ? 'Input link > Fetching link details' : 'Add the URL and we’ll handle the logo and meta tags for you')
