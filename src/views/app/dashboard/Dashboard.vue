<template>
  ...
  >
    <AppHeader
      ...
      :title="headerTitle"
      :browser-filters="browserFilters"
      @search="onSearch"
      @share="onShare"
      @browser-select="onBrowserSelect"
    />

    <PinnedBookmarks
      :bookmarks="pinnedCards"
      @context-action="onPinnedContextAction"
      @edit="openEditModal"
...
const pinnedCards = ref([
  ...
  },
  {
    ...
    time: "3:36pm",
    icon: "💬",
    bg: "bg-linear-to-br from-indigo-400 to-blue-300",
  },
  {
    title: "Telegram",
    ...
    time: "3:36pm",
    icon: "💬",
    bg: "bg-linear-to-br from-yellow-400 to-blue-300",
  },
]);

const allBookmarks = ref([
  {
    title: "Facebook - Connects with friends",
    url: "www.facebook.com",
    folder: "unsorted",
    time: "3:36pm",
    icon: "👤",
...
function onBrowserSelect(id) {
  browserFilters.value = browserFilters.value.map((f) => ({
    ...f,
    active: f.id === id,
  }));
}
