<template>
  <div
    class="w-full max-w-70 p-4 rounded-2xl bg-white border border-stone-200 shadow-sm space-y-4"
  >
    <!-- Set custom time -->
    <div class="space-y-1.5">
      <label class="block text-xs font-medium text-stone-700"
        >Set custom time</label
      >
      <div class="flex items-center gap-1.5">
        ...
        />
        <div
          class="flex rounded-full border border-stone-200 overflow-hidden shrink-0"
        >
          <button
            type="button"
    ...
    <div class="space-y-1.5">
      ...
      <input
        ...
        v-model="localDate"
        type="date"
        class="mt-1.5 w-full px-3 py-2 text-sm rounded-lg border border-stone-200"
    ...
    <div class="space-y-1.5">
      <label
        for="custom-sync-timezone"
        class="block text-xs font-medium text-stone-700"
        >Timezone</label
      >
      <div class="relative">
        <select
          id="custom-sync-timezone"
        ...
        <svg
          class="absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-400 pointer-events-none"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
...
const props = defineProps({
  time: {
    type: Object,
    default: () => ({ hours: "07", minutes: "00", amPm: "AM" }),
  },
...
watch(
  () => props.date,
  (v) => {
    localDate.value = v || toDateInputValue(new Date());
  },
);
watch(
  () => props.timezone,
  (v) => {
    localTimezone.value = v;
  },
);
...
function clampMinutes() {
  ...
  if (v === "" || isNaN(n)) localTime.value.minutes = v;
    ...
    localTime.value.minutes = String(Math.min(59, Math.max(0, n))).padStart(
      2,
      "0",
    );
}
