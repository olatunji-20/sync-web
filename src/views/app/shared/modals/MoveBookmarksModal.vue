    ...
    >
      ...
      >
        ...
        <div class="flex items-start justify-between gap-4 px-6 pt-6 pb-4">
          <div>
            <h2
              id="move-bookmarks-title"
              class="text-lg font-medium text-stone-800"
            >
              Move bookmarks
            </h2>
            <p class="text-sm text-stone-500 mt-0.5">
              Choose a collection to move your bookmarks to.
            </p>
          </div>
          <button
            type="button"
        ...
        <div class="px-4 max-h-64 overflow-y-auto">
          ...
          >
            ...
            <span class="text-sm font-medium text-stone-700 shrink-0"
              >{{ col.items }} items</span
            >
          </button>
        </div>
...
watch(
  ...
  (visible) => {
    if (visible) {
      ...
      const match = props.collections.find(
        (c) =>
          c.id === currentFolder ||
          c.name.toLowerCase().replace(/\s/g, "") ===
            (currentFolder || "").toLowerCase().replace(/\s/g, ""),
      );
...
function onMove() {
  ...
  emit("move", {
    targetCollectionId: selectedId.value,
